...
\begin{enumerate}
    ...
    \item \textbf{Out-of-vocabulary hallucination} (1 case): Rule AIT-0427---``\textit{If you score above a 6.0 on the AWPT, you will automatically be exempted from having to take mandatory EL courses}''---was assigned the type ``exemption'' by the LLM, a category not in the valid set (\texttt{obligation}/\texttt{permission}/\texttt{prohibition}). The classification node's output validation (Section~4.4) only sanitises empty or ``none'' values; it does not enforce membership in the valid category set. Linguistically, this sentence grants a \textit{conditional permission} (relief from an obligation) and should be classified as ``permission''. This is the only case across all 443 rules where the LLM hallucinated a novel category, indicating that output schema enforcement is otherwise effective but should include explicit set-membership validation as a defensive measure.
\end{enumerate}
...
\begin{itemize}
    ...
    \item LLM classification accuracy across the full D1 corpus (86.9\%, Cohen's $\kappa$ = 0.709, Macro F1 = 0.810),
...
The 73.6\% obligation rate reflects the pipeline's classification behavior: institutional policies primarily specify what stakeholders \textit{must} do, and the LLM exhibits a documented ``obligation bias'' that overestimates obligation prevalence. The post-audit gold distribution (Table~\ref{tab:deontic-type-breakdown}) shows 308 obligations (69.5\%), 75 permissions (16.9\%), and 60 prohibitions (13.5\%). The pipeline-assigned distribution in this table (326/50/66) is reported because SHACL shapes and FOL formulas are built from the \textit{pipeline} labels, not the gold labels. Category-specific classification challenges are analyzed in Section~\ref{sec:irr-type}.
