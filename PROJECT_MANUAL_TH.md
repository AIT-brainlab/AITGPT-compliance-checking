...
10. [ไฟล์ Output ที่โปรแกรมสร้างขึ้น](#10-ไฟล์-output-ที่โปรแกรมสร้างขึ้น)
11. [คำศัพท์สำคัญ (Glossary)](#11-คำศัพท์สำคัญ)
12. [คำถาม-คำตอบที่พบบ่อย](#12-คำถามคำตอบที่พบบ่อย)
...
```bash
python -m langgraph_agent.run --source institutional_policy/AIT --corpus ait
```

**อธิบาย argument:**
- `--source` — โฟลเดอร์ที่มี PDF ต้นฉบับ
- `--corpus ait` — ชื่อ corpus ซึ่งจะโหลด `config/ait.yaml`

### ดู Log แบบละเอียด

```bash
python -m langgraph_agent.run --source institutional_policy/AIT --corpus ait --verbose
```
...
python -m langgraph_agent.run --source institutional_policy/AIT --corpus ait --ablation no-prefilter

# รันทุก ablation พร้อมกัน
python -m langgraph_agent.run --source institutional_policy/AIT --corpus ait --ablation all
```
...
#### Ablation Flag:
```bash
ABLATION_SKIP_FOL_RETRY=1  # ไม่ retry → ถ้าล้มเหลวครั้งแรกให้เก็บไว้ใน fol_failed เลย
```
...
SHAPES_FILE      = OUTPUT_DIR / "generated_shapes.ttl"
...
python -m langgraph_agent.run \
    --source institutional_policy/AIT \
    --corpus ait \
    --ablation no-prefilter
```
...
| `no-hints` | `ABLATION_NO_VOCAB_HINTS=1` | ไม่บอก LLM เกี่ยวกับ vocabulary ที่มี |
...
| `no-fol-retry` | `ABLATION_SKIP_FOL_RETRY=1` | ไม่ retry FOL เมื่อ parse ไม่ได้ |
| `no-may-disambig` | `ABLATION_SKIP_MAY_DISAMBIG=1` | ไม่แยก epistemic vs deontic "may" |
| `all` | (รัน ablation ทั้งหมดต่อเนื่อง) | เปรียบเทียบทุก scenario |
...
## 10. ไฟล์ Output ที่โปรแกรมสร้างขึ้น
